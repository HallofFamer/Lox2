...
- Capability of saving bytecode into disk as .loxo file, which can be loaded later to avoid recomiling of the same source file.
...
## Documentation
[Lox2's documentation](https://mysidia-inc.gitbook.io/lox2) can be found on gitbook, which explores the features in Lox2 as well as some concrete example Lox2 programs. It is still a work in progress and receives updates regularly, but it should be a good reference for developers who want to learn about Lox2.

At this moment only the web version is available, though in Lox2 v2.3.0 onwards, Lox2 may have PDF documentation to download for interested developers. 
...
- Improved type system with non-nullable by default for type declaration, as well as covariant method parameter/return types.
...
```
git clone -b v2.2.2 https://github.com/HallofFamer/Lox2.git
cd Lox2
...
docker run -w /Lox2-2.2.2/Lox2 -i -t lox2:linux
```

Note: It is recommended to clone from the latest stable release branch("v2.2.2" at this moment), as the master branch receives updates regularly and some changes may break existing code. 
