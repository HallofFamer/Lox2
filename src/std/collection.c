...
static bool collectionIsEmpty(VM* vm, ObjInstance* collection) {
    int length = AS_INT(getObjField(vm, collection, "length"));
    return (length == 0);
}

static void collectionLengthDecrement(VM* vm, ObjInstance* collection) {
    int length = AS_INT(getObjField(vm, collection, "length"));
    setObjField(vm, collection, "length", INT_VAL(length - 1));
}
...
static bool linkIndexIsValid(VM* vm, ObjInstance* linkedList, int index) {
    int length = AS_INT(getObjField(vm, linkedList, "length"));
...
static ObjNode* linkNode(VM* vm, ObjInstance* linkedList, int index) {
    int length = AS_INT(getObjField(vm, linkedList, "length"));
    if (index < (length >> 1)) {
        ObjNode* node = AS_NODE(getObjField(vm, linkedList, "first"));
        ...
        ObjNode* node = AS_NODE(getObjField(vm, linkedList, "last"));
...
static int linkSearchElement(VM* vm, ObjInstance* linkedList, Value element) {
    int length = AS_INT(getObjField(vm, linkedList, "length"));
    if (length > 0) {
        ObjNode * first = AS_NODE(getObjField(vm, linkedList, "first"));
...
static ObjString* linkToString(VM* vm, ObjInstance* linkedList) {
    int size = AS_INT(getObjField(vm, linkedList, "length"));
...
static ObjInstance* setCopy(VM* vm, ObjInstance* original) {
    ObjDictionary* dict = AS_DICTIONARY(getObjField(vm, original, "dict"));
    ...
    setObjField(vm, copied, "dict", OBJ_VAL(dict2));
    return copied;
}

static ObjInstance* setDifference(VM* vm, ObjInstance* set1, ObjInstance* set2) {
	ObjDictionary* dict1 = AS_DICTIONARY(getObjField(vm, set1, "dict"));
	ObjDictionary* dict2 = AS_DICTIONARY(getObjField(vm, set2, "dict"));
	ObjDictionary* differenceDict = newDictionary(vm);
	push(vm, OBJ_VAL(differenceDict));
	...
	for (int i = 0; i < dict1->capacity; i++) {
		...
		if (!dictContainsKey(dict2, key)) {
			dictSet(vm, differenceDict, key, entry->value);
		}
	}

	pop(vm);
	ObjInstance* differenceSet = newInstance(vm, getNativeClass(vm, "clox.std.collection.Set"));
	setObjField(vm, differenceSet, "dict", OBJ_VAL(differenceDict));
...
static bool setSubsetOf(VM* vm, ObjInstance* set1, ObjInstance* set2) {
	ObjDictionary* dict1 = AS_DICTIONARY(getObjField(vm, set1, "dict"));
	ObjDictionary* dict2 = AS_DICTIONARY(getObjField(vm, set2, "dict"));
...
LOX_METHOD(Dictionary, entrySet) {
    ...
    setObjField(vm, entrySet, "dict", OBJ_VAL(entryDict));
    RETURN_OBJ(entrySet);
}
...
LOX_METHOD(Dictionary, keySet) {
    ...
    setObjField(vm, keySet, "dict", OBJ_VAL(keyDict));
    RETURN_OBJ(keySet);
}
...
LOX_METHOD(Dictionary, valueSet) {
    ...
    setObjField(vm, valueSet, "dict", OBJ_VAL(valueDict));
    RETURN_OBJ(valueSet);
}
...
LOX_METHOD(LinkedList, __init__) {
    ...
    setObjField(vm, self, "last", NIL_VAL);
    RETURN_OBJ(self);
}
...
LOX_METHOD(LinkedList, getFirst) {
    ...
    ObjNode* first = AS_NODE(getObjField(vm, AS_INSTANCE(receiver), "first"));
    RETURN_VAL(first->element);
}

LOX_METHOD(LinkedList, getLast) {
    ASSERT_ARG_COUNT("LinkedList::getLast()", 0);
    ObjNode* last = AS_NODE(getObjField(vm, AS_INSTANCE(receiver), "last"));
...
LOX_METHOD(LinkedList, length) {
    ...
    Value length = getObjField(vm, AS_INSTANCE(receiver), "length");
    RETURN_INT(length);
}
...
LOX_METHOD(LinkedList, remove) {
    ...
    ObjInstance* self = AS_INSTANCE(receiver);
    Value first = getObjField(vm, self, "first");
...
LOX_METHOD(LinkedList, removeLast) {
    ...
    ObjInstance* self = AS_INSTANCE(receiver);
    Value last = getObjField(vm, self, "last");
...
LOX_METHOD(LinkedListIterator, moveNext) {
    ...
    self->position++;
    if (IS_NIL(self->value)) self->value = getObjField(vm, linkedList, "first");
    else {
...
LOX_METHOD(Queue, __init__) {
    ...
    setObjField(vm, self, "last", OBJ_VAL(newNode(vm, NIL_VAL, NULL, NULL)));
    RETURN_OBJ(receiver);
}
...
LOX_METHOD(Set, difference) {
	ASSERT_ARG_COUNT("Set::difference(other)", 1);
	ASSERT_ARG_INSTANCE_OF("Set::difference(other)", 0, clox.std.collection.Set);
	ObjInstance* self = AS_INSTANCE(receiver);
...
LOX_METHOD(Set, equals) {
    ...
    ObjDictionary* dict = AS_DICTIONARY(getObjField(vm, self, "dict"));
    ObjDictionary* dict2 = AS_DICTIONARY(getObjField(vm, AS_INSTANCE(args[0]), "dict"));
...
LOX_METHOD(Set, subsetOf) {
	ASSERT_ARG_COUNT("Set::subsetOf(other)", 1);
	ASSERT_ARG_INSTANCE_OF("Set::subsetOf(other)", 0, clox.std.collection.Set);
	ObjInstance* self = AS_INSTANCE(receiver);
	ObjInstance* other = AS_INSTANCE(args[0]);
	RETURN_BOOL(setSubsetOf(vm, self, other));
}
...
LOX_METHOD(Stack, pop) {
    ...
    if (length == 0) RETURN_NIL;
        ...
        Value element = first->element;
        setObjField(vm, self, "first", first->next == NULL ? NIL_VAL : OBJ_VAL(first->next));
        collectionLengthDecrement(vm, self);
        RETURN_VAL(element);
    }
}
...
void registerCollectionPackage(VM* vm) {
    ...
    DEF_INTERCEPTOR(collectionClass, Collection, INTERCEPTOR_INIT, __init__, 0, NATIVE_TYPE(clox.std.collection.Collection));
    ...
    DEF_METHOD(collectionClass, Collection, length, 0, NATIVE_TYPE(Int));
    ...
    DEF_METHOD(listClass, List, getAt, 1, NATIVE_TYPE(E), NATIVE_TYPE(Int));
    DEF_METHOD(listClass, List, putAt, 2, NATIVE_TYPE(void), NATIVE_TYPE(Int), NATIVE_TYPE(E));

    bindSuperclass(vm, vm->arrayClass, listClass);
    vm->arrayClass->classType = OBJ_ARRAY;
    DEF_INTERCEPTOR(vm->arrayClass, Array, INTERCEPTOR_INIT, __init__, 0, NATIVE_TYPE(clox.std.collection.Array));
    DEF_METHOD(vm->arrayClass, Array, add, 1, NATIVE_TYPE(clox.std.collection.Array), NATIVE_TYPE(E));
    DEF_METHOD(vm->arrayClass, Array, addAll, 1, NATIVE_TYPE(void), NATIVE_TYPE(clox.std.collection.Collection));
    ...
    DEF_METHOD(vm->arrayClass, Array, eachIndex, 1, NATIVE_TYPE(void), NATIVE_TYPE_CALLABLE(NATIVE_TYPE(void), 2, NATIVE_TYPE(Int), NATIVE_TYPE(E)));
    DEF_METHOD(vm->arrayClass, Array, equals, 1, NATIVE_TYPE(Bool), NATIVE_TYPE(Object));
    DEF_METHOD(vm->arrayClass, Array, fill, 2, NATIVE_TYPE(void), NATIVE_TYPE(Int), NATIVE_TYPE(E));
    DEF_METHOD(vm->arrayClass, Array, getAt, 1, NATIVE_TYPE(E), NATIVE_TYPE(Int));
    DEF_METHOD(vm->arrayClass, Array, indexOf, 1, NATIVE_TYPE(Int), NATIVE_TYPE(E));
    DEF_METHOD(vm->arrayClass, Array, insertAt, 2, NATIVE_TYPE(void), NATIVE_TYPE(Int), NATIVE_TYPE(E));
    DEF_METHOD(vm->arrayClass, Array, isEmpty, 0, NATIVE_TYPE(Bool));
    DEF_METHOD(vm->arrayClass, Array, iterator, 0, NATIVE_TYPE(clox.std.collection.ArrayIterator));
    DEF_METHOD(vm->arrayClass, Array, lastIndexOf, 1, NATIVE_TYPE(Int), NATIVE_TYPE(E));
    DEF_METHOD(vm->arrayClass, Array, length, 0, NATIVE_TYPE(Int));
    DEF_METHOD(vm->arrayClass, Array, putAt, 2, NATIVE_TYPE(void), NATIVE_TYPE(Int), NATIVE_TYPE(E));
    ...
    DEF_OPERATOR(vm->arrayClass, Array, [], __getSubscript__, 1, NATIVE_TYPE(E), NATIVE_TYPE(Int));
    ...
    DEF_METHOD(arrayIteratorClass, ArrayIterator, moveNext, 0, NATIVE_TYPE(Bool));
    ...
    DEF_INTERCEPTOR(linkedListClass, LinkedList, INTERCEPTOR_INIT, __init__, 0, NATIVE_TYPE(clox.std.collection.LinkedList));
    DEF_METHOD(linkedListClass, LinkedList, add, 1, NATIVE_TYPE(clox.std.collection.LinkedList), NATIVE_TYPE(E));
    DEF_METHOD(linkedListClass, LinkedList, addAt, 2, NATIVE_TYPE(E), NATIVE_TYPE(Int), NATIVE_TYPE(E));
    DEF_METHOD(linkedListClass, LinkedList, addFirst, 1, NATIVE_TYPE(void), NATIVE_TYPE(E));
    DEF_METHOD(linkedListClass, LinkedList, addLast, 1, NATIVE_TYPE(void), NATIVE_TYPE(E));
    DEF_METHOD(linkedListClass, LinkedList, clear, 0, NATIVE_TYPE(void));
    DEF_METHOD(linkedListClass, LinkedList, contains, 1, NATIVE_TYPE(Bool), NATIVE_TYPE(E));
    DEF_METHOD(linkedListClass, LinkedList, getAt, 1, NATIVE_TYPE(E), NATIVE_TYPE(Int));
    DEF_METHOD(linkedListClass, LinkedList, getFirst, 0, NATIVE_TYPE(E));
    DEF_METHOD(linkedListClass, LinkedList, getLast, 0, NATIVE_TYPE(E));
    DEF_METHOD(linkedListClass, LinkedList, indexOf, 1, NATIVE_TYPE(Int), NATIVE_TYPE(E));
    DEF_METHOD(linkedListClass, LinkedList, isEmpty, 0, NATIVE_TYPE(Bool));
    DEF_METHOD(linkedListClass, LinkedList, iterator, 0, NATIVE_TYPE(clox.std.collection.LinkedListIterator));
    DEF_METHOD(linkedListClass, LinkedList, lastIndexOf, 0, NATIVE_TYPE(Int), NATIVE_TYPE(E));
    DEF_METHOD(linkedListClass, LinkedList, length, 0, NATIVE_TYPE(Int));
    DEF_METHOD(linkedListClass, LinkedList, node, 1, NATIVE_TYPE(E), NATIVE_TYPE(Int));
    DEF_METHOD(linkedListClass, LinkedList, peek, 0, NATIVE_TYPE(E));
    DEF_METHOD(linkedListClass, LinkedList, putAt, 2, NATIVE_TYPE(void), NATIVE_TYPE(Int), NATIVE_TYPE(E));
    DEF_METHOD(linkedListClass, LinkedList, remove, 0, NATIVE_TYPE(E));
    DEF_METHOD(linkedListClass, LinkedList, removeFirst, 0, NATIVE_TYPE(E));
    DEF_METHOD(linkedListClass, LinkedList, removeLast, 0, NATIVE_TYPE(E));
    DEF_METHOD(linkedListClass, LinkedList, toArray, 0, NATIVE_TYPE(clox.std.collection.Array));
    DEF_METHOD(linkedListClass, LinkedList, toString, 0, NATIVE_TYPE(String));

    bindSuperclass(vm, linkedListIteratorClass, vm->iteratorClass);
    DEF_INTERCEPTOR(linkedListIteratorClass, LinkedListIterator, INTERCEPTOR_INIT, __init__, 1, NATIVE_TYPE(clox.std.collection.LinkedListIterator), NATIVE_TYPE(Object));
    DEF_METHOD(linkedListIteratorClass, LinkedListIterator, currentValue, 0, NATIVE_TYPE(Object));
    DEF_METHOD(linkedListIteratorClass, LinkedListIterator, moveNext, 0, NATIVE_TYPE(Bool));

    bindSuperclass(vm, vm->nodeClass, vm->objectClass);
    vm->nodeClass->classType = OBJ_NODE;
    DEF_FIELD(vm->nodeClass, element, E, true, NIL_VAL);
    ...
    DEF_INTERCEPTOR(vm->nodeClass, Node, INTERCEPTOR_INIT, __init__, 3, NATIVE_TYPE(clox.std.collection.Node), NATIVE_TYPE(E), NATIVE_TYPE(clox.std.collection.Node), NATIVE_TYPE(clox.std.collection.Node));
    DEF_METHOD(vm->nodeClass, Node, clone, 0, NATIVE_TYPE(clox.std.collection.Node));
    DEF_METHOD(vm->nodeClass, Node, element, 0, NATIVE_TYPE(E));
    DEF_METHOD(vm->nodeClass, Node, next, 0, NATIVE_TYPE(clox.std.collection.Node));
    DEF_METHOD(vm->nodeClass, Node, prev, 0, NATIVE_TYPE(clox.std.collection.Node));
    DEF_METHOD(vm->nodeClass, Node, toString, 0, NATIVE_TYPE(String));

    bindSuperclass(vm, vm->dictionaryClass, collectionClass);
    vm->dictionaryClass->classType = OBJ_DICTIONARY;
    DEF_INTERCEPTOR(vm->dictionaryClass, Dictionary, INTERCEPTOR_INIT, __init__, 0, NATIVE_TYPE(clox.std.collection.Dictionary));
    ...
    DEF_METHOD(vm->dictionaryClass, Dictionary, each, 1, NATIVE_TYPE(void), NATIVE_TYPE_CALLABLE(NATIVE_TYPE(void), 2, NATIVE_TYPE(K), NATIVE_TYPE(V)));
    DEF_METHOD(vm->dictionaryClass, Dictionary, eachKey, 1, NATIVE_TYPE(void), NATIVE_TYPE_CALLABLE(NATIVE_TYPE(void), 1, NATIVE_TYPE(K)));
    DEF_METHOD(vm->dictionaryClass, Dictionary, eachValue, 1, NATIVE_TYPE(void), NATIVE_TYPE_CALLABLE(NATIVE_TYPE(void), 1, NATIVE_TYPE(V)));
    DEF_METHOD(vm->dictionaryClass, Dictionary, entrySet, 0, NATIVE_TYPE(clox.std.collection.Set));
    DEF_METHOD(vm->dictionaryClass, Dictionary, equals, 1, NATIVE_TYPE(Bool), NATIVE_TYPE(Object));
    DEF_METHOD(vm->dictionaryClass, Dictionary, getAt, 1, NATIVE_TYPE(V), NATIVE_TYPE(K));
    DEF_METHOD(vm->dictionaryClass, Dictionary, isEmpty, 0, NATIVE_TYPE(Bool));
    DEF_METHOD(vm->dictionaryClass, Dictionary, iterator, 0, NATIVE_TYPE(clox.std.collection.DictionaryIterator));
    DEF_METHOD(vm->dictionaryClass, Dictionary, length, 0, NATIVE_TYPE(Int));
    DEF_METHOD(vm->dictionaryClass, Dictionary, keySet, 0, NATIVE_TYPE(clox.std.collection.Set));
    DEF_METHOD(vm->dictionaryClass, Dictionary, putAll, 1, NATIVE_TYPE(void), NATIVE_TYPE(clox.std.collection.Dictionary));
    DEF_METHOD(vm->dictionaryClass, Dictionary, putAt, 2, NATIVE_TYPE(void), NATIVE_TYPE(K), NATIVE_TYPE(V));
    DEF_METHOD(vm->dictionaryClass, Dictionary, reject, 1, NATIVE_TYPE(clox.std.collection.Dictionary), NATIVE_TYPE_CALLABLE(NATIVE_TYPE(Bool), 2, NATIVE_TYPE(K), NATIVE_TYPE(V)));
    DEF_METHOD(vm->dictionaryClass, Dictionary, removeAt, 1, NATIVE_TYPE(V), NATIVE_TYPE(K));
    DEF_METHOD(vm->dictionaryClass, Dictionary, select, 1, NATIVE_TYPE(clox.std.collection.Dictionary), NATIVE_TYPE_CALLABLE(NATIVE_TYPE(Bool), 2, NATIVE_TYPE(K), NATIVE_TYPE(V)));
    DEF_METHOD(vm->dictionaryClass, Dictionary, toArray, 0, NATIVE_TYPE(clox.std.collection.Array));
    DEF_METHOD(vm->dictionaryClass, Dictionary, toString, 0, NATIVE_TYPE(String));
    DEF_METHOD(vm->dictionaryClass, Dictionary, valueSet, 0, NATIVE_TYPE(clox.std.collection.Set));
    DEF_OPERATOR(vm->dictionaryClass, Dictionary, [], __getSubscript__, 1, NATIVE_TYPE(V), NATIVE_TYPE(K));
    DEF_OPERATOR(vm->dictionaryClass, Dictionary, []=, __setSubscript__, 2, NATIVE_TYPE(void), NATIVE_TYPE(K), NATIVE_TYPE(V));

    bindSuperclass(vm, dictionaryIteratorClass, vm->iteratorClass);
    DEF_INTERCEPTOR(dictionaryIteratorClass, DictionaryIterator, INTERCEPTOR_INIT, __init__, 1, NATIVE_TYPE(clox.std.collection.DictionaryIterator), NATIVE_TYPE(Object));
    DEF_METHOD(dictionaryIteratorClass, DictionaryIterator, currentIndex, 0, NATIVE_TYPE(Object));
    DEF_METHOD(dictionaryIteratorClass, DictionaryIterator, moveNext, 0, NATIVE_TYPE(Bool));

    bindSuperclass(vm, vm->entryClass, vm->objectClass);
    vm->entryClass->classType = OBJ_ENTRY;
    DEF_FIELD(vm->entryClass, key, K, true, NIL_VAL);
    DEF_FIELD(vm->entryClass, value, V, true, NIL_VAL);
    DEF_INTERCEPTOR(vm->entryClass, Entry, INTERCEPTOR_INIT, __init__, 2, NATIVE_TYPE(clox.std.collection.Entry), NATIVE_TYPE(K), NATIVE_TYPE(V));
    DEF_METHOD(vm->entryClass, Entry, clone, 0, NATIVE_TYPE(clox.std.collection.Entry));
    DEF_METHOD(vm->entryClass, Entry, getKey, 0, NATIVE_TYPE(K));
    DEF_METHOD(vm->entryClass, Entry, getValue, 0, NATIVE_TYPE(V));
    DEF_METHOD(vm->entryClass, Entry, setValue, 1, NATIVE_TYPE(void), NATIVE_TYPE(K));
    DEF_METHOD(vm->entryClass, Entry, toString, 0, NATIVE_TYPE(String));
    ...
    DEF_INTERCEPTOR(setClass, Set, INTERCEPTOR_INIT, __init__, 0, NATIVE_TYPE(clox.std.collection.Set));
    DEF_METHOD(setClass, Set, add, 1, NATIVE_TYPE(Bool), NATIVE_TYPE(E));
    DEF_METHOD(setClass, Set, clear, 0, NATIVE_TYPE(void));
    ...
    DEF_METHOD(setClass, Set, subsetOf, 1, NATIVE_TYPE(Bool), NATIVE_TYPE(clox.std.collection.Set));
    ...
    DEF_METHOD(setIteratorClass, SetIterator, currentIndex, 0, NATIVE_TYPE(Object));
    DEF_METHOD(setIteratorClass, SetIterator, moveNext, 0, NATIVE_TYPE(Bool));
    ...
    DEF_INTERCEPTOR(vm->rangeClass, Range, INTERCEPTOR_INIT, __init__, 2, NATIVE_TYPE(clox.std.collection.Range), NATIVE_TYPE(Int), NATIVE_TYPE(Int));
    DEF_METHOD(vm->rangeClass, Range, add, 1, NATIVE_TYPE(clox.std.collection.Range), NATIVE_TYPE(Object));
    DEF_METHOD(vm->rangeClass, Range, addAll, 1, NATIVE_TYPE(void), NATIVE_TYPE(clox.std.collection.Collection));
    DEF_METHOD(vm->rangeClass, Range, clone, 0, NATIVE_TYPE(clox.std.collection.Range));
    DEF_METHOD(vm->rangeClass, Range, contains, 1, NATIVE_TYPE(Bool), NATIVE_TYPE(Object));
    DEF_METHOD(vm->rangeClass, Range, from, 0, NATIVE_TYPE(Int));
    DEF_METHOD(vm->rangeClass, Range, getAt, 1, NATIVE_TYPE(Int), NATIVE_TYPE(Int));
    DEF_METHOD(vm->rangeClass, Range, iterator, 0, NATIVE_TYPE(clox.std.collection.RangeIterator));
    DEF_METHOD(vm->rangeClass, Range, length, 0, NATIVE_TYPE(Int));
    DEF_METHOD(vm->rangeClass, Range, max, 0, NATIVE_TYPE(Int));
    DEF_METHOD(vm->rangeClass, Range, min, 0, NATIVE_TYPE(Int));
    DEF_METHOD(vm->rangeClass, Range, reverse, 0, NATIVE_TYPE(clox.std.collection.Range));
    DEF_METHOD(vm->rangeClass, Range, step, 2, NATIVE_TYPE(void), NATIVE_TYPE(Int), NATIVE_TYPE_CALLABLE(NATIVE_TYPE(void), 1, NATIVE_TYPE(Int)));
    DEF_METHOD(vm->rangeClass, Range, to, 0, NATIVE_TYPE(Int));
    DEF_METHOD(vm->rangeClass, Range, toArray, 0, NATIVE_TYPE(clox.std.collection.Array));
    DEF_METHOD(vm->rangeClass, Range, toString, 0, NATIVE_TYPE(String));

    bindSuperclass(vm, rangeIteratorClass, vm->iteratorClass);
    DEF_INTERCEPTOR(rangeIteratorClass, RangeIterator, INTERCEPTOR_INIT, __init__, 1, NATIVE_TYPE(clox.std.collection.RangeIterator), NATIVE_TYPE(Object));
    DEF_METHOD(rangeIteratorClass, RangeIterator, moveNext, 0, NATIVE_TYPE(Bool));

    bindSuperclass(vm, stackClass, collectionClass);
    DEF_FIELD(stackClass, first, clox.std.collection.Node, true, NIL_VAL);
    DEF_INTERCEPTOR(stackClass, Stack, INTERCEPTOR_INIT, __init__, 0, NATIVE_TYPE(clox.std.collection.Stack));
    DEF_METHOD(stackClass, Stack, clear, 0, NATIVE_TYPE(void));
    DEF_METHOD(stackClass, Stack, contains, 1, NATIVE_TYPE(Bool), NATIVE_TYPE(E));
    DEF_METHOD(stackClass, Stack, getFirst, 0, NATIVE_TYPE(E));
    DEF_METHOD(stackClass, Stack, isEmpty, 0, NATIVE_TYPE(Bool));
    DEF_METHOD(stackClass, Stack, iterator, 0, NATIVE_TYPE(clox.std.collection.StackIterator));
    DEF_METHOD(stackClass, Stack, length, 0, NATIVE_TYPE(Int));
    DEF_METHOD(stackClass, Stack, peek, 0, NATIVE_TYPE(E));
    DEF_METHOD(stackClass, Stack, pop, 0, NATIVE_TYPE(E));
    DEF_METHOD(stackClass, Stack, push, 1, NATIVE_TYPE(void), NATIVE_TYPE(E));
    DEF_METHOD(stackClass, Stack, search, 1, NATIVE_TYPE(Int), NATIVE_TYPE(E));
    DEF_METHOD(stackClass, Stack, toArray, 0, NATIVE_TYPE(clox.std.collection.Array));
    DEF_METHOD(stackClass, Stack, toString, 0, NATIVE_TYPE(String));

    bindSuperclass(vm, stackIteratorClass, linkedListIteratorClass);
    DEF_INTERCEPTOR(stackIteratorClass, StackIterator, INTERCEPTOR_INIT, __init__, 1, NATIVE_TYPE(clox.std.collection.StackIterator), NATIVE_TYPE(Object));
    ...
    DEF_INTERCEPTOR(queueClass, Queue, INTERCEPTOR_INIT, __init__, 0, NATIVE_TYPE(clox.std.collection.Queue));
    DEF_METHOD(queueClass, Queue, clear, 0, NATIVE_TYPE(void));
    DEF_METHOD(queueClass, Queue, contains, 1, NATIVE_TYPE(Bool), NATIVE_TYPE(E));
    DEF_METHOD(queueClass, Queue, dequeue, 0, NATIVE_TYPE(E));
    DEF_METHOD(queueClass, Queue, enqueue, 1, NATIVE_TYPE(void), NATIVE_TYPE(E));
    DEF_METHOD(queueClass, Queue, getFirst, 0, NATIVE_TYPE(E));
    DEF_METHOD(queueClass, Queue, getLast, 0, NATIVE_TYPE(E));
    DEF_METHOD(queueClass, Queue, isEmpty, 0, NATIVE_TYPE(Bool));
    DEF_METHOD(queueClass, Queue, iterator, 0, NATIVE_TYPE(clox.std.collection.QueueIterator));
    DEF_METHOD(queueClass, Queue, length, 0, NATIVE_TYPE(Int));
    DEF_METHOD(queueClass, Queue, peek, 0, NATIVE_TYPE(E));
    DEF_METHOD(queueClass, Queue, search, 1, NATIVE_TYPE(Bool), NATIVE_TYPE(E));
    DEF_METHOD(queueClass, Queue, toArray, 0, NATIVE_TYPE(clox.std.collection.Array));
    DEF_METHOD(queueClass, Queue, toString, 0, NATIVE_TYPE(String));

    bindSuperclass(vm, queueIteratorClass, linkedListIteratorClass);
    DEF_INTERCEPTOR(queueIteratorClass, QueueIterator, INTERCEPTOR_INIT, __init__, 1, NATIVE_TYPE(clox.std.collection.QueueIterator), NATIVE_TYPE(Object));
