...
static Token keywordIdentifier(Lexer* lexer) {
    ...
    if (peek(lexer) == '`') {
        advance(lexer);
        return makeToken(lexer, TOKEN_KIND_IDENTIFIER);
    }
    else {
...
static Token number(Lexer* lexer) {
    ...
    if (peek(lexer) == '.' && isDigit(peekNext(lexer))) {
        ...
        return makeToken(lexer, TOKEN_KIND_NUMBER);
    }
    return makeToken(lexer, TOKEN_KIND_INT);
}
...
static Token string(Lexer* lexer) {
    while ((peek(lexer) != '"' || peekPrevious(lexer) == '\\') && !isAtEnd(lexer)) {
        if (peek(lexer) == '\n') lexer->line++;
            ...
            advance(lexer);
            Token token = makeToken(lexer, TOKEN_KIND_INTERPOLATION);
            advance(lexer);
            return token;
        }
        advance(lexer);
    ...
    advance(lexer);
    return makeToken(lexer, TOKEN_KIND_STRING);
}

static Token newLine(Lexer* lexer) {
    lexer->line++;
    return makeToken(lexer, TOKEN_KIND_NEW_LINE);
}

Token scanToken(Lexer* lexer) {
    skipWhitespace(lexer);
    lexer->start = lexer->current;
    if (isAtEnd(lexer)) return makeToken(lexer, TOKEN_KIND_EOF);
    ...
    switch (c) {
        case '(': return makeToken(lexer, TOKEN_KIND_LEFT_PAREN);
        case ')': return makeToken(lexer, TOKEN_KIND_RIGHT_PAREN);
        case '[': return makeToken(lexer, TOKEN_KIND_LEFT_BRACKET);
        case ']': return makeToken(lexer, TOKEN_KIND_RIGHT_BRACKET);
        case '{': return makeToken(lexer, TOKEN_KIND_LEFT_BRACE);
        case '}':
            if (lexer->interpolationDepth > 0) {
                lexer->interpolationDepth--;
                return string(lexer);
            }
            return makeToken(lexer, TOKEN_KIND_RIGHT_BRACE);
        case ';': return makeToken(lexer, TOKEN_KIND_SEMICOLON);
        case ':': return makeToken(lexer, TOKEN_KIND_COLON);
        case ',': return makeToken(lexer, TOKEN_KIND_COMMA);
        case '?': return makeToken(lexer, TOKEN_KIND_QUESTION);
        case '-': return makeToken(lexer, TOKEN_KIND_MINUS);
        case '%': return makeToken(lexer, TOKEN_KIND_MODULO);
        case '|': return makeToken(lexer, TOKEN_KIND_PIPE);
        case '+': return makeToken(lexer, TOKEN_KIND_PLUS);
        case '/': return makeToken(lexer, TOKEN_KIND_SLASH);
        case '*': return makeToken(lexer, TOKEN_KIND_STAR);
        case '!': 
            return makeToken(lexer, match(lexer, '=') ? TOKEN_KIND_BANG_EQUAL : TOKEN_KIND_BANG);
        case '=': 
            return makeToken(lexer, match(lexer, '=') ? TOKEN_KIND_EQUAL_EQUAL : TOKEN_KIND_EQUAL);
        case '>':
            return makeToken(lexer, match(lexer, '=') ? TOKEN_KIND_GREATER_EQUAL : TOKEN_KIND_GREATER);
        case '<':
            return makeToken(lexer, match(lexer, '=') ? TOKEN_KIND_LESS_EQUAL : TOKEN_KIND_LESS);
        case '.':
            return makeToken(lexer, match(lexer, '.') ? TOKEN_KIND_DOT_DOT : TOKEN_KIND_DOT);
        case '`': 
