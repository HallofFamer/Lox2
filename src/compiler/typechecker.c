...
static TypeInfo* insertHigherOrderType(TypeChecker* typeChecker, TypeInfo* type) {
	if (isHigherOrderType(type)) {
        ...
        TypeInfo* existingType = typeTableGet(typeChecker->vm->typetab, type->fullName);
...
static CallableTypeInfo* instantiateGenericFunctionType(TypeChecker* typeChecker, TypeInfo* type) {
    ...
    for (int i = 0; i < functionType->paramTypes->count; i++) {
        ...
        if (hasGenericParameters(paramType)) {
            paramType = instantiateTypeParameterWithName(typeChecker, paramType, functionType->formalTypeParams, genericFunctionType->actualTypeParams);
        }
        TypeInfoArrayAdd(instantiatedFunctionType->paramTypes, paramType);
    }
    
	return AS_CALLABLE_TYPE(insertHigherOrderType(typeChecker, (TypeInfo*)instantiatedFunctionType));
}
...
static CallableTypeInfo* instantiateGenericMethodTypeFromAst(TypeChecker* typeChecker, CallableTypeInfo* declaredType, Ast* typeParams) {
	...
	return AS_CALLABLE_TYPE(insertHigherOrderType(typeChecker, (TypeInfo*)instantiatedCallableType));
}
...
static CallableTypeInfo* instantiateGenericMethodType(TypeChecker* typeChecker, TypeInfo* behaviorType, TypeInfo* methodType) {
    ...
    CallableTypeInfo* rawMethodType = AS_CALLABLE_TYPE(getInnerBaseType(methodType));
	...
	return AS_CALLABLE_TYPE(insertHigherOrderType(typeChecker, (TypeInfo*)instantiatedMethodType));
}
...
static TypeInfo* getAstGenericDynamicType(TypeChecker* typeChecker, Ast* ast, TypeInfo* type) {
	...
	return insertHigherOrderType(typeChecker, (TypeInfo*)genericType);
}
...
static void checkTypeParameters(TypeChecker* typeChecker, Ast* ast, TypeInfo* type) {
    ...
    Ast* actualTypeParams = astGetChild(ast, 0);

    if (actualTypeParams->children->count != behaviorType->formalTypeParams->count) {
        typeError(typeChecker, "Class %s expects to receive %d generic type parameters but gets %d.",
            type->shortName->chars, behaviorType->formalTypeParams->count, actualTypeParams->children->count);
    }
}
...
static void inheritGenericSupertypeMethods(TypeChecker* typeChecker, BehaviorTypeInfo* subtype, GenericTypeInfo* supertype) {
    ...
    for (int i = 0; i < behaviorType->methods->capacity; i++) {
        ...
        if (entry != NULL && entry->key != NULL) {
            ...
            subMethodType->declaredType = AS_CALLABLE_TYPE(insertHigherOrderType(typeChecker, (TypeInfo*)subMethodType->declaredType));
...
static void inferAstTypeFromBinaryOperator(TypeChecker* typeChecker, Ast* ast, SymbolItem* item) {
    ...
    if (receiver->type == NULL || IS_PLACEHOLDER_TYPE(receiver->type) || arg->type == NULL) return;
...
static void inferAstTypeFromInitializer(TypeChecker* typeChecker, Ast* ast, TypeInfo* type) {
    ...
    if (initializerType != NULL) {
        CallableTypeInfo* callableType = instantiateGenericMethodType(typeChecker, callee->type, (TypeInfo*)(AS_METHOD_TYPE(initializerType)->declaredType));
...
static void inferAstTypeFromCall(TypeChecker* typeChecker, Ast* ast) {
    ...
    if (callee->kind == AST_EXPR_FUNCTION) {
        ...
        CallableTypeInfo* calleeType = newCallableTypeInfo(typeChecker->vm->typetab->count + 1, TYPE_CATEGORY_CALLABLE, emptyString(typeChecker->vm), NULL);
...
static void inferAstTypeFromInvoke(TypeChecker* typeChecker, Ast* ast) {
    ...
    if (baseType != NULL) {
        ...
        ObjString* _namespace = nameTableGet(typeChecker->nametab, createStringFromToken(typeChecker->vm, receiver->token));
...
static void inferAstTypeFromSuperInvoke(TypeChecker* typeChecker, Ast* ast) {
    ...
    if (methodType->declaredType->formalTypeParams->count > 0) {
        ...
        Ast* typeParams = astLastChild(ast);
        if (typeParams->children->count != methodType->declaredType->formalTypeParams->count) {
            typeError(typeChecker, "Method %s::%s expects to receive %d generic type parameters but gets %d.", superType->shortName->chars,
                methodName->chars, methodType->declaredType->formalTypeParams->count, typeParams->children->count);
            return;
        }
...
static void inferAstTypeFromSubscriptGet(TypeChecker* typeChecker, Ast* ast) {
    ...
    if (receiver->type == NULL || IS_PLACEHOLDER_TYPE(receiver->type) || index->type == NULL) return;
...
static void typeCheckThis(TypeChecker* typeChecker, Ast* ast) {
    if (typeChecker->currentClass->type) {
        if (typeChecker->currentFunction->isClass) {
            ObjString* className = getClassFullName(typeChecker->vm, createStringFromToken(typeChecker->vm, typeChecker->currentClass->name), typeChecker->currentNamespace);
...
static void typeCheckClassDeclaration(TypeChecker* typeChecker, Ast* ast) {
    ObjString* className = createStringFromToken(typeChecker->vm, ast->token);
    ObjString* metaclassName = getMetaclassNameFromClass(typeChecker->vm, className);
...
static void typeCheckMethodDeclaration(TypeChecker* typeChecker, Ast* ast) {
    SymbolItem* item = symbolTableLookup(ast->symtab, createStringFromToken(typeChecker->vm, ast->token));
...
static void typeCheckNamespaceDeclaration(TypeChecker* typeChecker, Ast* ast) {
    ...
    SymbolItem* item = symbolTableGet(ast->symtab, createStringFromToken(typeChecker->vm, identifier->token));
...
static void typeCheckTraitDeclaration(TypeChecker* typeChecker, Ast* ast) {
    SymbolItem* item = symbolTableGet(ast->symtab, createStringFromToken(typeChecker->vm, ast->token));
